...
class CurlHandleTest extends \Guzzle\Tests\GuzzleTestCase
{
     ...
     */
    public function testFactoryCreatesCurlBasedOnRequest(
        $method,
        $url,
        $headers,
        $body,
        ...
        foreach ($options as $key => $value) {
            ...
            if ($key != CURLOPT_HTTPHEADER && $key != CURLOPT_POSTFIELDS && ( is_array( $o[$key] ) ) || $o[$key] instanceof \Closure) {
                ...
                $this->assertTrue( $value == $o[$key],
                    '-> Check number '.$check.' - '.var_export( $value, true ).' != '.var_export( $o[$key], true ) );
            }
        }
        ...
        if ($expectedHeaders) {
            ...
            if ($request->getHeader( 'Content-Length' )) {
                $this->assertEquals( (string)$request->getHeader( 'Content-Length' ),
                    (string)$requests[0]->getHeader( 'Content-Length' ) );
            }
        }
    }
    ...
    public function testSendsPostRequestsWithFiles()
    {
        ...
        if (version_compare( phpversion(), '5.5.0', '<' )) {
            $this->assertContains( '@'.__FILE__.';filename=CurlHandleTest.php;type=text/x-',
                $options[CURLOPT_POSTFIELDS]['foo'] );
        } else {
