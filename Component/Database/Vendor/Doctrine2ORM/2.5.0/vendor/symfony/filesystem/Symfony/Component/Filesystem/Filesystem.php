    ...
    public function symlink( $originDir, $targetDir, $copyOnWindows = false )
    {
        ...
        if (!$ok) {
            if (true !== @symlink( $originDir, $targetDir )) {
                ...
                if (is_array( $report )) {
                    if (defined( 'PHP_WINDOWS_VERSION_MAJOR' ) && false !== strpos( $report['message'],
                            'error code(1314)' )
                    ) {
            ...
            if (!file_exists( $targetDir )) {
                throw new IOException( sprintf( 'Symbolic link "%s" is created but appears to be broken.', $targetDir ),
                    0, null, $targetDir );
            }
        }
    }
