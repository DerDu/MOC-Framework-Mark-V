...
class QueryBuilderTest extends \Doctrine\Tests\DbalTestCase
{
    ...
    public function testReferenceJoinFromJoin()
    {
        ...
        $this->setExpectedException( 'Doctrine\DBAL\Query\QueryException',
            "The given alias 'invalid' is not part of any FROM or JOIN clause table. The currently registered aliases are: news, nv." );
    ...
    public function testSelectWithMultipleFromAndJoins()
    {
        ...
        $this->assertEquals( 'SELECT DISTINCT u.id FROM users u INNER JOIN permissions p ON p.user_id = u.id, articles a INNER JOIN comments c ON c.article_id = a.id WHERE (u.id = a.user_id) AND (p.read = 1)',
            $qb->getSQL() );
    }
    ...
    public function testComplexSelectWithoutTableAliases()
    {
        ...
        $this->assertEquals( 'SELECT DISTINCT users.id FROM users INNER JOIN permissions p ON p.user_id = users.id, articles INNER JOIN comments c ON c.article_id = articles.id WHERE (users.id = articles.user_id) AND (p.read = 1)',
            $qb->getSQL() );
    }
    ...
    public function testComplexSelectWithSomeTableAliases()
    {
        ...
        $this->assertEquals( 'SELECT u.id FROM users u INNER JOIN permissions p ON p.user_id = u.id, articles INNER JOIN comments c ON c.article_id = articles.id',
            $qb->getSQL() );
    }
    ...
    public function testGetParameterTypes()
    {
        ...
        $this->assertSame( array( 'name' => \PDO::PARAM_STR, 'isActive' => \PDO::PARAM_BOOL ),
