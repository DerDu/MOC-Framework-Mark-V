...
class Twig_Tests_Node_Expression_GetAttrTest extends Twig_Test_NodeTestCase
{
    ...
    public function getTests()
    {
        ...
        $tests[] = array(
            $node,
            sprintf('%s%s, "bar", array(), "array")', $this->getAttributeGetter(),
        ...
        $tests[] = array(
            $node,
            sprintf('%s%s, "bar", array(0 => %s, 1 => "bar"), "method")', $this->getAttributeGetter(),
                $this->getVariableGetter('foo', 1), $this->getVariableGetter('foo'))
        );
