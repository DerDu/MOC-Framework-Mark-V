...
abstract class Twig_Node_Expression_Call extends Twig_Node_Expression
{
    ...
    protected function compileCallable(Twig_Compiler $compiler)
    {
        ...
        if ($this->hasAttribute('callable') && $callable = $this->getAttribute('callable')) {
            if (is_string($callable)) {
                ...
                $compiler->raw(sprintf('$this->env->getExtension(\'%s\')->%s', $callable[0]->getName(),
                    $callable[1]));
            } else {
                $type = ucfirst($this->getAttribute('type'));
                $compiler->raw(sprintf('call_user_func_array($this->env->get%s(\'%s\')->getCallable(), array', $type,
    ...
    protected function getArguments($callable, $arguments)
    {
        ...
        if (!$callable) {
            throw new LogicException(sprintf('Named arguments are not supported for %s "%s".', $callType,
                $callName));
        }
            ...
            if (array_key_exists($name, $parameters)) {
                ...
                if (!empty( $missingArguments )) {
                    throw new Twig_Error_Syntax(sprintf(
                            ...
                            $name, $callType, $callName, implode(', ', $names),
                            count($missingArguments) > 1 ? 's' : '', implode('", "', $missingArguments))
                    );
                }
